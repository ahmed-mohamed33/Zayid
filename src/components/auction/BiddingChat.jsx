...
const BiddingChat = ({ auctionId }) => {
  ...
  const handleBidSubmit = async () => {
    if (
      Number(bidAmount) > 0 &&
      Number(bidAmount) > Number(bids[bids.length - 1].amount)
    ) {
  ...
  return (
    <div
      className="bg-white rounded-2xl border border-[#BFC0C0] p-6 w-full mt-8"
      dir="rtl"
    >
      ...
      {user.isAdmin && (
        <div className="flex justify-center items-center mt-4 w-full">
          <button
            ...
            className="bg-[#44A46F] hover:bg-[#4f8c6b] text-white font-bold px-4 py-3 rounded-lg transition-colors duration-200 w-[340px] justify-items-center"
          >
            انهاء المزاد
          </button>
        </div>

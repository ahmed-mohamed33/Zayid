...
export const UserProvider = ({ children }) => {
  ...
  // Update User Data
  const updateUserData = async (uid, updatedFields) => {
    const db = getDatabase();
    const usersRef = ref(db, 'users');
    ...
    // Find the correct user key using uid
    const userEntry = Object.entries(data).find(
      ([_, userData]) => userData.userId === uid
    );

    if (!userEntry) throw new Error('User not found for update');

    const [userKey] = userEntry;

    // Now update the right path
      ...
      if (currentUser) {
        ...
        onValue(usersRef, (snapshot) => {
          ...
          const userEntry = Object.entries(allUsers).find(
            ([_, userData]) => userData.userId === currentUser.uid
          );
